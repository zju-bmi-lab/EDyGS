...
class thrmodel:
    def __init__(self):
        self.thr = nn.Parameter(torch.tensor(0.2))
        self.optimizer = None
    ...
    def clip_thr(self):
        self.thr.data = self.thr.data.clamp(0.1, 1)
...
class DeformModel:
    ...
    # def step(self, xyz, time_emb):
    #     self.step_counter += 1
    #     if self.step_counter < 3000 and self.step_counter % 90 == 0:
    #         self.freeze_gradients()
    #     else:
    #         self.unfreeze_gradients()
    #     output = self.deform(xyz, time_emb)
    #     return output
