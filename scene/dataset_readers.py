...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        intrinsics = np.array([[focal_length_x, 0, width/2], [0, focal_length_x, height/2], [0, 0, 1]])
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, intrinsics=intrinsics, image=image,
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    if eval:
        test_indices = set()
        for i in range(0, len(cam_infos), 40):  # 每40个元素开始一个新的测试集
            test_indices.update(range(i, min(i + 5, len(cam_infos))))  # 获取从i开始的5个元素索引
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            intrinsics = np.array([[focal_length_x, 0, image.size[0]/2], [0, focal_length_x, image.size[1]/2], [0, 0, 1]])

            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, intrinsics=intrinsics, image=image,
